...
qInfo=dir('query'); %name属性にファイル名が格納
query_num=length(qInfo)-2; %ファイルの数
tInfo=dir('target'); %name属性にファイル名が格納
target_num=length(tInfo)-2; %ファイルの数

if(query_num==0 || target_num==0)
    fprintf('queryフォルダ or targetフォルダ にファイルがありません\n')
    return
end
